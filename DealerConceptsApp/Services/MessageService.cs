...
namespace DealerConceptsApp.Services
{
    public class MessageService : BaseService, IMessageService, IAdminMessageService
    {
        ...
        private static SendGridClient _sg = new SendGridClient("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");
        ...
        public string GetEmailContent(EmailTemplateKind issue, List<KeyValuePair<string, string>> tokens)
        {
            ...
            switch (issue)
            {
                case EmailTemplateKind.NewFindNotification:
                    body = File.ReadAllText(HttpContext.Current.Server.MapPath("/Views/EmailTemplates/NewFindNotification.html"));
                    break;
                case EmailTemplateKind.NewRequest:
                    body = File.ReadAllText(HttpContext.Current.Server.MapPath("/Views/EmailTemplates/NewRequest.html"));
                    break;
                case EmailTemplateKind.NewChat:
                    body = File.ReadAllText(HttpContext.Current.Server.MapPath("/Views/EmailTemplates/ConfirmationEmail.html"));
                    break;
                case EmailTemplateKind.NewConfirmChat:
                    body = File.ReadAllText(HttpContext.Current.Server.MapPath("/Views/EmailTemplates/RecoveryEmail.html"));
                    break;
                case EmailTemplateKind.NewRescheduledChat:
                    body = File.ReadAllText(HttpContext.Current.Server.MapPath("/Views/UserRegistration/ConfirmationEmail.html"));
                    break;
                case EmailTemplateKind.NoNewChat:
                    body = File.ReadAllText(HttpContext.Current.Server.MapPath("/Views/UserRegistration/ConfirmationEmail.html"));
                    break;
                case EmailTemplateKind.NewCustomerToAdmin:
                    body = File.ReadAllText(HttpContext.Current.Server.MapPath("/Views/EmailTemplates/NewCustomerToAdminEmail.html "));
                    break;
                case EmailTemplateKind.SendRequestRespondedNotification:
                    body = File.ReadAllText(HttpContext.Current.Server.MapPath("/Views/EmailTemplates/RequestResponded.html"));
                    break;
                default:
